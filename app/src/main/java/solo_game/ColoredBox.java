...
public class ColoredBox extends GameItem {
    ...
    private static final Vector3f defaultColor = new Vector3f(128, 0, 0);
    ...
    public void isCollided(Camera camera) {
        ...
        for (int i = 0; i < elementArray.length; i += 3) {
            ...
            int b = elementArray[i + 1];
            int c = elementArray[i + 2];
            if (new TrianglePlane(points[a], points[b], points[c]).isCollided(ray)) {
                isCollided = true;
                return;
            }
        }
